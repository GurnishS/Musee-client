...
class PlayerCubit extends Cubit<PlayerViewState> {
  ...
  PlayerCubit({PlayerRepository? repository})
      : _player = AudioPlayer(),
        _repo = repository,
  ...
  // Resolve playable URL similar to UI helpers
  ...
  Future<void> playFromQueueTrackId(String trackId) async {
    ...
    if (repo != null) {
      try {
        final expanded = await repo.playQueueFrom(trackId: trackId, expand: true);
        final items = expanded.map((m) => QueueItem.fromExpandedJson(m)).toList();
        emit(state.copyWith(queue: items));
      } catch (_) {}
    }
    ...
    if (idx >= 0) {
      ...
      if (url != null) {
        await playTrack(PlayerTrack(url: url, title: 'Unknown', artist: 'Unknown'));
      }
    }
  }
  ...
  Future<void> addToQueue(List<QueueItem> items) async {
    ...
    final repo = _repo;
    if (repo != null) {
      unawaited(repo.addToQueue(trackIds: items.map((e) => e.trackId).toList()));
    }
  }
